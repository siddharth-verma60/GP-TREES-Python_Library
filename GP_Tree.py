...
Parameters and functions used to describe the tree are described as follows:"""
    ...
    def __init__(self, function_set=("add", "mul", "sub", "div", "cos", "sqrt", "absolute", "sin", "tan"),\
                 terminal_set=[random.randint(-5, 5)], num_features = 1, min_depth = 2, max_depth = 3, \
        ...
        X0, X1, X2), max and min depth values and the fitness metric to be used. Three common fitness metrics have
        already been defined in "Fitness.py"."""
        ...
        ###################################################################
        # The parameters as it is taken from the ExSTraCS code.
    ...
    def setPhenotype(self, args):
        ...
        if dataInfo.discretePhenotype and len(dataInfo.phenotypeList) == 2:
            ...
            if not self.epochComplete:  # Not sure where Ben came up with this but it seems to makes reasonable sense.
                # May want to examine more carefully.
                ...
                # For trees there could be one uniform random chance. Ultimately we want to use balanced accuracy for
                # trees (do we do better than by chance) but for now we will just use 0.5 for simplicity.
        ...
        # -------------------------------------------------------
            ...
            if self.evaluate(args) < 0:  # lowest class
                ...
                notfoundClass = True
                while notfoundClass:
                    if count > self.evaluate(args) >= count - 1:
            ...
            if not self.epochComplete:  # Not sure where Ben came up with this but it seems to makes reasonable sense.
                # May want to examie more carefully.
        ...
        # -------------------------------------------------------
            ...
            self.phenotype = float(self.evaluate(args)) # Typecasting was done because self.evaluate returned np array.
            if not self.epochComplete:  # Not sure where Ben came up with this but it seems to makes reasonable sense.
                # May want to examine more carefully.
                self.phenotype_RP = 0.5
    ...
    def getDelProp(self, meanFitness):
        ...
        if self.fitness/self.numerosity >= cons.delta*meanFitness or self.matchCount < cons.theta_del:
            ...
            # Note: numerosity seems redundant (look into theory of deletion in LCS).
        return self.deletionVote

    #####################################################################################
    #                           Subsumption Methods (From Exstracs)                     #
    #####################################################################################
    ...
    #####################################################################################
    #                    Parameters Update (Taken from exstracs code)                   #
    #####################################################################################
    ...
    def updateEpochStatus(self, exploreIter):
        ...
        if not self.epochComplete and (exploreIter - self.initTimeStamp) >= cons.env.formatData.numTrainInstances and cons.offlineData:
            ...
            #randomProbClass = cons.env.formatData.classProportions[self.phenotype]
            # #Put this in as a fix - for rules that become epoch complete after having been extrapolated on a previous
            # run.
...
#####################################################################################
#           Tree Cloning: Similar to the func: "treeClone" in exstracs              #
#####################################################################################
...
def crossover_onepoint(first_parent, second_parent):
    """ This method performs the one point crossover in the population. It prepares a replica copy of the parents and
    then searches for the nodes in the common subtrees of the selected parents and then choose a random nodes from both
    the trees and then swap them between esch other."""

    parent1 = first_parent
...
def crossover_onepoint_helper(node1, node2, common_subtree1_nodes, common_subtree2_nodes):
    """ This is a helper function for crossover that recursively checks for the common subtrees between two GP trees
    according to the common shapes. It populates the lists in its arguments with the common nodes in the respective
    trees.
    """

    # Base condition in case of a terminal nodes
    if node1.children is None or node2.children is None:  # arity==0
        return
...
def uniformCrossover(first_parent, second_parent, state):
    ...
    if first_parent.isTree and second_parent.isTree:  # BOTH TREES-use the above implemented one-point crossover method.
        ...
        try:
            crossover_onepoint(first_parent, second_parent)  # Method defined just above!
        except:
    ...
    if str(origForm1) == str(first_parent) and (isinstance(second_parent, GP_Tree) and str(origForm2) == str(second_parent)):
    ...
    while len(tree_cross) != 0 or len(rule_cross) != 0:
        ...
        if len(tree_cross) == 0:  # Add an extra feature to tree
            ...
            try:
                ...
                if random.choice([0, 1]) == 0:  # Case 1: Add the terminals in the tree.
                    if not replace_constant(terminals, rule_arg):
                        add_terminal(tree, rule_arg)
                else:  # Case 2: Replace the constants in the tree with the feature.
                    if not add_terminal(tree, rule_arg):
                        replace_constant(terminals, rule_arg)
...
def remove_from_tree(tree: GP_Tree, terminals: list, index: int):
    removed = []
...
def replace_constant(terminals: list, ruleAtt: int) -> bool:
...
def add_terminal(tree: GP_Tree, ruleAtt: int) -> bool:
...
def all_nodes_helper(node, all_nodes):
    ...
    for child in node.children:
        function_nodes_helper(child, all_nodes)  # Recursive calls for the other nodes.
...
if __name__=="__main__":
    ...
    # For adding ephemeral constant into the terminal_set, add a random integer in the tuple passed in the following call.
    tree = GP_Tree(function_set=("add", "mul", "sub", "div", "cos", "sqrt", "absolute", "sin", "tan"),
                   terminal_set=(7.5, 11.0, random.randint(-5, 5)), num_features=1, min_depth=2, max_depth=4,
                   fitness_metric="Root_Mean_Square_Error")
    ...
    parent = tournament_selection(population, k=1, tournament_size=5)
    ...
    offspring = mutation_Uniform(*parent, mut_subtree)
    ...
    offsprings = crossover_onepoint(*parents)
